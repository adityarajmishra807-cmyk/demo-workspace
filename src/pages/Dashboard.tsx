...
import { getClientSummaries, deleteClient, isProtectedClient } from '@/data/clientRegistry';
import type { TemplateId } from '@/types/client';
import { LayoutDashboard, Plus, ExternalLink, ArrowLeft, Pencil, Trash2, X } from 'lucide-react';
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        <div className="mb-10">
          ...
          <p className="text-white/50 text-sm max-w-2xl">
            This is the internal dashboard for managing client website demos. Each demo is generated from a configuration file — no client has its own separate codebase.
          </p>
        </div>
        ...
        <div className="mb-8">
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-sky-500 hover:bg-sky-400 text-white font-semibold text-sm transition-all duration-300 hover:scale-105 shadow-lg shadow-sky-500/20"
          >
            <Plus size={18} />
            Create New Demo
          </button>
              ...
              return (
                <div key={s.id} className="grid grid-cols-12 gap-4 px-6 py-4 border-t border-white/5 items-center hover:bg-white/[0.03] transition-colors">
                  <div className="col-span-3 font-medium text-sm">
                    {s.businessName}
                    {protectedClient && (
                      <span className="ml-2 text-[10px] uppercase tracking-wider text-white/30 bg-white/5 px-1.5 py-0.5 rounded">test</span>
                    )}
                  </div>
